...
import datetime
import struct
from usb import BaseStucture, USBDevice, InterfaceDescriptor, DeviceConfigurations, EndPoint, USBContainer
import cwiid
...
interface_d.endpoints = [EndPoint(bEndpointAddress=0x81,
                     bmAttributes=0x3,
                     wMaxPacketSize=0x0800,  # Little endian
                     bInterval=0xA)  # interval to report]  # Supports only one endpoint
configuration.interfaces = [InterfaceDescriptor(bAlternateSetting=0,
                                  bNumEndpoints=1,
                                  bInterfaceClass=0x03,  # class HID
                                  bInterfaceSubClass=0,
                                  bInterfaceProtocol=0, #joystick
                                  iInterface=0)]   # Supports only one interface
